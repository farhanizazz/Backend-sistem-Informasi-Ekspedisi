...
class WordHelper
{
    ...
    public function generateWord(array $payload)
    {
        ...
        $values = $data['transaksi_tagihan_det']->map(function ($transaksi_tagihan_det) {
            // biaya = harga_order + biaya_tambah_kurang
            $jumlahPembayaran = $this->hitungTotalBiayaLain($transaksi_tagihan_det->transaksi_order->biaya_lain_harga_jual_arr) + $this->hitungTotalBiayaLain($transaksi_tagihan_det->transaksi_order->biaya_lain_harga_order_arr) + $this->hitungTotalBiayaLain($transaksi_tagihan_det->transaksi_order->biaya_lain_uang_jalan_arr);
            $biaya = $transaksi_tagihan_det->transaksi_order->harga_order + $jumlahPembayaran;
            $totalMutasiOrder = $transaksi_tagihan_det->transaksi_order->mutasi_order->sum('nominal');
            $sisaTagihan = $biaya - $totalMutasiOrder;
            $this->total += $sisaTagihan;
            ...
            return [
                ...
                'sisa_tagihan' => $this->rupiah($sisaTagihan),
            ];
        });
